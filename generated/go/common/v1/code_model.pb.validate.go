...
// ValidateAll checks the field values on DataContainerId with the rules
// defined in the proto definition for this message. If any rules are
// violated, the result is a list of violation errors wrapped in
// DataContainerIdMultiError, or nil if none found.
func (m *DataContainerId) ValidateAll() error {
	return m.validate(true)
}

func (m *DataContainerId) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if len(m.GetValue()) != 16 {
		err := DataContainerIdValidationError{
			field:  "Value",
			reason: "value length must be 16 bytes",
		}
		if !all {
	...
	if len(errors) > 0 {
		return DataContainerIdMultiError(errors)
	}

	return nil
}

// DataContainerIdMultiError is an error wrapping multiple validation errors
// returned by DataContainerId.ValidateAll() if the designated constraints
// aren't met.
type DataContainerIdMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m DataContainerIdMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m DataContainerIdMultiError) AllErrors() []error { return m }
...
func (m *DeviceToken) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetValue()); l < 1 || l > 16348 {
		err := DeviceTokenValidationError{
			field:  "Value",
